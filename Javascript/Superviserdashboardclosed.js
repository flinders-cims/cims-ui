document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchClosedSR() {
        const supervisor_closed_data= localStorage.getItem('supervisor_closed_data');
         // Parse the JSON string back into an object
const parsedData = JSON.parse(supervisor_closed_data);
    ...
    function populateTable(serviceRequests) {
        ...
        if (!Array.isArray(serviceRequests) || serviceRequests.length === 0) {
            ...
            row.innerHTML = `<td colspan="5">No closed Service Requests Found</td>`;
